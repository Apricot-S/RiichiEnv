...
def _compute_ranks(scores: list, n_players: int) -> list[int]:
    """Compute ranks (0=1st, n-1=last) from scores."""
    arr = np.array(scores[:n_players], dtype=np.float64)
...
class RVWorker:
    ...
    Parameterized by n_players, game_mode, tile_dim for 3P/4P support.
    """
    ...
    def _warmup_compile(self):
        ...
        if hasattr(target, "a_head"):
            ...
            num_actions = GAME_PARAMS[self.n_players]["num_actions"]
    ...
    def collect_episodes(self):
        ...
        kyoku_buffers = [{pid: [] for pid in range(n)} for _ in range(self.num_envs)]
        ...
        while any(active):
            ...
            for ei in range(self.num_envs):
                ...
                if cur_kyoku_idx != prev_kyoku_idx[ei] and any(kyoku_buffers[ei][p] for p in range(n)):
                    cur_scores = list(env.scores())
                    ranks = _compute_ranks(cur_scores, n)
                ...
                if env.done():
                    if any(kyoku_buffers[ei][p] for p in range(n)):
                        cur_scores = list(env.scores())
                        ranks = _compute_ranks(cur_scores, n)
                        rw, oya, honba, rsticks = kyoku_start_meta[ei]
                        all_rewards = self._compute_kyoku_rewards(
